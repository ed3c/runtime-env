...
|---|---|---|---|---|
| `agy-gemini36-flash-high-replay` | `/Users/neon/skill-bettor` | `skill-bettor-agy-replay`; no carrier policy | Public replay-contract test, model inventory, then the approved file-based replay. | `BLOCKED` above L2: the public replay test now executes and passes, but sending the replay payload to the logged-in external model requires explicit data-transmission authority. No current L4/L5 receipt exists. |
...
| `repo-wiki-converge` | `/Users/neon/ix-agy` | `ix-agy-repo-wiki`; no carrier policy | Fixed author/verify/ingest adapter plus consumer and adapter public tests; mutable output is broker-owned. | `BLOCKED` above L2: `REPO_WIKI_RUN_ROOT` is user-owned mode `0700`, the explicit Python runtime is configured, and 14 consumer tests plus the adapter control pass. Live authoring would transmit repository content to an external model and has no explicit authority. |
