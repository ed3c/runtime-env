...
def registry(path: Path):
    ...
    for name,row in repos.items():
        ...
        for n,phases in row["required_phases"].items():
            if not n.isdigit() or not isinstance(phases,list) or len(phases)!=len(set(phases)) or any(p not in PHASES for p in phases): raise MonitorError(f"invalid required phases {name}#{n}")
    return v
...
def get_json(url,token):
    ...
    try:
        with urllib.request.urlopen(req,timeout=30) as r: # noqa: S310 origin checked above
            data=json.loads(r.read().decode()); meta={"rate_limit":r.headers.get("X-RateLimit-Limit"),"rate_remaining":r.headers.get("X-RateLimit-Remaining")}
...
def pages(url,token):
    ...
    while True:
        sep="&" if "?" in url else "?"; data,meta=get_json(f"{url}{sep}per_page=100&page={page}",token)
...
def normalize_issue(repo,issue,required):
...
def build_packet_from_fixture(reg,repo,issues,deps):
    if repo not in reg["repositories"]: raise MonitorError(f"repository not allowlisted: {repo}")
    if not isinstance(issues,list) or not isinstance(deps,dict): raise MonitorError("fixture shapes are invalid")
    by_id={}
    def add(r,i):
        if not isinstance(i,dict): raise MonitorError("issue row must be an object")
        if "pull_request" in i: return None
        phases=reg["repositories"].get(r,{}).get("required_phases",{}).get(str(i.get("number")),[])
        item=normalize_issue(r,i,phases); ident=f"{r}#{item['number']}"; old=by_id.get(ident)
    ...
    for ident in initial:
        ...
        if ident not in deps: raise MonitorError(f"dependency response missing for open issue: {ident}")
        edges=[]
        for dep in deps[ident]:
            dep_repo=repo_from_url(dep.get("repository_url"))
            if dep_repo!=repo: raise MonitorError(f"cross-repository dependency needs its own exact monitor subject: {ident} -> {dep_repo}")
            dep_id=add(dep_repo,dep)
            if dep_id: edges.append(dep_id)
        by_id[ident]["depends_on"]=sorted(set(edges))
    return [by_id[k] for k in sorted(by_id)]

def fetch_packet(reg,repo,token):
    if repo not in reg["repositories"]: raise MonitorError(f"repository not allowlisted: {repo}")
    owner,name=repo.split("/",1); issues,page_meta=pages(f"{API}/repos/{owner}/{name}/issues?state=all",token); issues=[i for i in issues if "pull_request" not in i]
    deps={}; dep_meta=[]
    for issue in issues:
        if issue.get("state")!="open": continue
        n=issue.get("number"); ident=f"{repo}#{n}"; rows,meta=pages(f"{API}/repos/{owner}/{name}/issues/{n}/dependencies/blocked_by",token); deps[ident]=rows; dep_meta.extend({"issue":ident,**m} for m in meta)
...
def compile_plan(packet,skills_root,expected_commit):
    root=skills_root.resolve()
    if git_head(root)!=expected_commit: raise MonitorError("skills-shared exact subject mismatch")
    compiler=root/"skills/shared-skills-infra/scripts/repository_control_plane.py"; schema=root/"skills/shared-skills-infra/references/repository-control-plane-monitor-plan.v1.schema.json"
